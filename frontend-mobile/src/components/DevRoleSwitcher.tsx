...
export function DevRoleSwitcher() {
  ...
  const handleSeed = async () => {
    ...
    try {
      ...
      // Salva os tokens recebidos no localStorage
    ...
    <aside aria-label="Alternador de Perfis para Demonstração" className="fixed bottom-3 right-3 z-50">
      {!open ? (
        <button
          ...
          className="flex items-center gap-2 rounded-full border border-primary/30 bg-card/95 px-3 py-1.5 text-xs font-semibold text-foreground shadow-lg backdrop-blur hover:bg-accent"
        >
          <span className="size-2 rounded-full bg-success animate-pulse" />
          <span>Demo: {activeRole === "tutor" ? "👤 Tutor" : activeRole === "vet" ? "🩺 Vet" : "🛡️ RT"}</span>
        ...
        <div className="w-80 rounded-2xl border border-border bg-card/95 p-3.5 shadow-2xl backdrop-blur">
          <div className="mb-2.5 flex items-center justify-between border-b border-border/60 pb-2">
            <div>
              <p className="text-xs font-bold uppercase tracking-wider text-primary">Modo de Apresentação</p>
